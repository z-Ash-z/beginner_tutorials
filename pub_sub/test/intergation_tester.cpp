...
class TestingFixture : public testing::Test {
 public:
   ...
   */
  void SetUp() override {
    client_ = node_->create_client<pub_sub::srv::StringChange>(
        "change_publisher_string");
  ...
  std::string send_service_call() {
    ...
    request->new_string =
        "Let's go!";  // The request that will be sent via the service.
 ...
 protected:
  ...
  rclcpp::Client<pub_sub::srv::StringChange>::SharedPtr
